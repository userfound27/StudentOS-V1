...
function StudentOSApp({mode,onExit,onSignIn,onDeleteAccount}:{mode:"anonymous"|"account";onExit:()=>Promise<void>|void;onSignIn:()=>void;onDeleteAccount:()=>Promise<string>}){
 ...
 const [focusSeconds,setFocusSeconds]=useState(1500),[focusMinutes,setFocusMinutes]=useState(25),[focusRunning,setFocusRunning]=useState(false),[accountError,setAccountError]=useState("");
 useEffect(()=>{
...
function Focus(p:{seconds:number;minutes:number;setMinutes:(x:number)=>void;running:boolean;setRunning:(x:boolean)=>void;reset:()=>void}){const m=Math.floor(p.seconds/60).toString().padStart(2,"0"),s=(p.seconds%60).toString().padStart(2,"0");const [draft,setDraft]=useState(String(p.minutes));useEffect(()=>{if(!p.running)setDraft(String(p.minutes))},[p.minutes,p.running]);const applyDuration=(value:string)=>{setDraft(value);if(value==="")return;const numeric=Number(value);if(!Number.isFinite(numeric))return;const next=Math.min(180,Math.max(1,Math.floor(numeric)));p.setMinutes(next);if(!p.running)p.reset()};const commitDuration=()=>{const numeric=Number(draft);const next=Number.isFinite(numeric)&&numeric>0?Math.min(180,Math.max(1,Math.floor(numeric))):1;setDraft(String(next));p.setMinutes(next);if(!p.running)p.reset()};const step=(delta:number)=>{const current=Number(draft)||p.minutes;const next=Math.min(180,Math.max(1,current+delta));setDraft(String(next));p.setMinutes(next);if(!p.running)p.reset()};return <div className="focus-page"><div className="focus-card"><div className="hero-kicker"><Clock3 size={15}/> FOCUS MODE</div><h2>{m+":"+s}</h2><p>One focused block. One clear objective.</p><div className="focus-actions"><button className="primary-btn" onClick={()=>p.setRunning(!p.running)}>{p.running?"Pause":"Start focus"}</button><button className="ghost-btn" onClick={p.reset}>Reset</button></div><div className="focus-duration"><label htmlFor="focus-minutes">Session length</label><div className="duration-control"><button type="button" className="duration-step" aria-label="Decrease session length by 1 minute" disabled={p.running||p.minutes<=1} onClick={()=>step(-1)}>−</button><input id="focus-minutes" type="text" inputMode="numeric" pattern="[0-9]*" value={draft} disabled={p.running} onChange={e=>applyDuration(e.target.value.replace(/\\D/g,"").slice(0,3))} onBlur={commitDuration} onKeyDown={e=>{if(e.key==="Enter"){e.currentTarget.blur()}}} aria-describedby="focus-duration-help"/><button type="button" className="duration-step" aria-label="Increase session length by 1 minute" disabled={p.running||p.minutes>=180} onClick={()=>step(1)}>+</button><span>minutes</span></div><div className="duration-presets">{[15,25,45,60,90].map(v=><button key={v} type="button" className={p.minutes===v?"duration-preset active":"duration-preset"} disabled={p.running} onClick={()=>{setDraft(String(v));p.setMinutes(v);p.reset()}}>{v}m</button>)}</div></div><div className="focus-note" id="focus-duration-help"><Zap size={17}/> Choose any focus length from 1–180 minutes.</div></div></div>}
