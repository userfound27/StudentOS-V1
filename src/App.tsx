...
function StudentOSApp({mode,onExit,onSignIn,onDeleteAccount}:{mode:"anonymous"|"account";onExit:()=>Promise<void>|void;onSignIn:()=>void;onDeleteAccount:()=>Promise<string>}){
 ...
 useEffect(()=>{window.__studentosUpdate=async()=>{if(!("serviceWorker" in navigator)){window.location.reload();return;}const registration=await navigator.serviceWorker.getRegistration();if(!registration){window.location.reload();return;}await registration.update();const installing=registration.installing;if(installing){await new Promise<void>(resolve=>{const done=()=>{if(installing.state==="installed"||installing.state==="redundant"){installing.removeEventListener("statechange",done);resolve()}};installing.addEventListener("statechange",done);});}if(registration.waiting){registration.waiting.postMessage({type:"SKIP_WAITING"});await new Promise<void>(resolve=>{const timer=window.setTimeout(resolve,3000);navigator.serviceWorker.addEventListener("controllerchange",()=>{window.clearTimeout(timer);resolve()},{once:true})});}else{await new Promise(resolve=>window.setTimeout(resolve,350));}window.location.reload()};return()=>{delete window.__studentosUpdate}},[]);
...
function UpdateControl(){
 ...
 const check=async()=>{setStatus("checking");try{const before=navigator.serviceWorker?.controller?.scriptURL;await window.__studentosUpdate?.();setStatus("updated");void before;}catch{setStatus("error")}};
 const text=status==="checking"?"Checking for updates…":status==="updated"?"Update applied.":"If a new StudentOS deployment is available, this will install it and refresh the app.";
