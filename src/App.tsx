...
function StudentOSApp({mode,onExit,onSignIn,onDeleteAccount}:{mode:"anonymous"|"account";onExit:()=>Promise<void>|void;onSignIn:()=>void;onDeleteAccount:()=>Promise<string>}){
 ...
 return <div className="app-shell">
  <aside className={(mobileNav?"sidebar open ":"sidebar ")+(sidebarCollapsed?"collapsed":"")}>
   <div className="brand"><div className="brand-mark"><Command size={19}/></div><div><strong>StudentOS</strong><span>your school operating system</span></div><button className="icon-btn mobile-close" aria-label="Collapse sidebar" onClick={()=>setSidebarCollapsed(v=>!v)}><X size={18}/></button></div>
   <nav>
...
function Focus(p:{seconds:number;minutes:number;setMinutes:(x:number)=>void;running:boolean;setRunning:(x:boolean)=>void;reset:()=>void}){const m=Math.floor(p.seconds/60).toString().padStart(2,"0"),s=(p.seconds%60).toString().padStart(2,"0");const [draft,setDraft]=useState(String(p.minutes));useEffect(()=>{if(!p.running)setDraft(String(p.minutes))},[p.minutes,p.running]);const setDuration=(value:string)=>{setDraft(value);if(value==="")return;const numeric=Number(value);if(!Number.isFinite(numeric))return;const next=Math.min(180,Math.max(1,Math.floor(numeric)));p.setMinutes(next);if(!p.running)p.reset()};const commitDuration=()=>{if(draft.trim()===""){setDraft("1");p.setMinutes(1);if(!p.running)p.reset();return}const next=Math.min(180,Math.max(1,Math.floor(Number(draft)||1)));setDraft(String(next));p.setMinutes(next);if(!p.running)p.reset()};return <div className="focus-page"><div className="focus-card"><div className="hero-kicker"><Clock3 size={15}/> FOCUS MODE</div><h2>{m+":"+s}</h2><p>One focused block. One clear objective.</p><div className="focus-actions"><button className="primary-btn" onClick={()=>p.setRunning(!p.running)}>{p.running?"Pause":"Start focus"}</button><button className="ghost-btn" onClick={p.reset}>Reset</button></div><div className="focus-duration"><label htmlFor="focus-minutes">Session length</label><div><input id="focus-minutes" type="number" min="1" max="180" inputMode="numeric" value={draft} disabled={p.running} onChange={e=>setDuration(e.target.value)} onBlur={commitDuration} aria-describedby="focus-duration-help"/><span>minutes</span></div></div><div className="focus-note" id="focus-duration-help"><Zap size={17}/> Choose any focus length from 1–180 minutes.</div></div></div>}
